...
import {IGameManager} from "./abstracts/types/interfaces.ts";
...
import {
    ...
    pixiConfig,
    soundConfig
} from "./Minesweeper/config.ts";
import Minesweeper from "./Minesweeper/Minesweeper.ts";
import {BackgroundModule} from "./Minesweeper/modules/BackgroundModule.ts";
import {ConnectionModule} from "./Minesweeper/modules/ConnectionModule.ts";
import {GamesModule} from "./Minesweeper/modules/GamesModule.ts";
import {IntroModule} from "./Minesweeper/modules/IntroModule.ts";
import {SoundModule} from "./Minesweeper/modules/SoundModule.ts";
import {UiModule} from "./Minesweeper/modules/UiModule.ts";
import {GameName} from "./Minesweeper/types/enums.ts";
import {IGameManagerModules, IStateMachine} from "./Minesweeper/types/interfaces.ts";
...
export class GameManager implements IGameManager {
    ...
    protected setupSignalsHandlers(): void {
        ...
        signalsManager.on(
            ...
            async (gameName: GameName = GameName.MINESWEEPER) => {
        ...
        signalsManager.on(SignalNames.GAME_ENDED, async () => {
            await this.stateMachine.next(GameState.END_GAME);
        });

        signalsManager.on(SignalNames.GAME_RESTART, async () => {
            await this.modules.connection.generateCombination();
        ...
        const resizeCanvas = () => {
            ...
            if (width < height) {
                ...
                if (scaleFactor >= 1.4) {
                    ...
                    scale *= 1.4;
                } else if (scaleFactor > 1 && scaleFactor < 1.2) {
                    scale *= 0.96;
                }
            } else {
